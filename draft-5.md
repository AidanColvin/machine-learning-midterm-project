...
The goal of this project is to build a machine learning model that uses 13 clinical features to estimate the probability that an individual has heart disease. The model is trained on labeled synthetic data generated by a deep learning model trained on a real heart disease prediction dataset. Using synthetic data preserves patient privacy while enabling large-scale training. Probability estimates from this model may support clinicians in identifying high-risk patients and prioritizing targeted preventive treatment.
