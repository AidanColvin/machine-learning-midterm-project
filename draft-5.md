...
The goal of this project is to build a machine learning model that uses 13 clinical features to estimate the probability that an individual has heart disease. Probability estimates from this model may support clinicians in identifying high-risk patients and prioritizing targeted preventive treatment.

### Dataset Description

The training dataset contains 630,000 observations and the test dataset contains 270,000 observations. The training data was labeled synthetic data, generated by a deep learning model trained on a real heart disease prediction dataset. Using synthetic data preserves patient privacy while enabling large-scale training. Each observation includes 13 predictor features and one binary response variable, ***Heart Disease***, encoded as 0 (absence) and 1 (presence). There are no missing values in either dataset. The 13 predictor features span continuous, binary, and ordinal types:
